		...
		try {
			...
			if (this.getTitle() == "修改") {// 如果是修改记录
				...
				try {
					...
					int rs1 = stmt.executeUpdate("UPDATE students SET sName='"+xm+"',sSex='"+xb.trim()+"',sDept='"+yx.trim()+"',sAge='"+nl+"' where sId='"+xh.trim()+"'");
...
//			}
		...
		if (this.getTitle() == "增加") {// 如果是增加记录，对应的用户表中也增加一条记录
			try {
				this.connDB();
				stmt.executeUpdate("insert into students(sId,sName,sAge,sSex,sDept) values('" + xh + "','" + xm + "'," + nl
					+ ",'" + xb + "','" + yx + "')");
